...
async def StreamResponses(question:str,thinking:bool):
    ...
    agent=create_react_agent(
        ...
        prompt="""
            ...
            **Important** Warn the Users all the time when they ask a question about that: You (AI Assistant) is only a help that can get real-time and historical stock price data, but the User needs to do researches if investing (this AI is not a Financial Advisor, Just a help for Assisting to get the real time  and historical stock prices) 
        """
    )
...
def get_question(question:question):
    return StreamingResponse(StreamResponses(question.question,False),media_type="text/event_stream")
